...
(()=>{
 ...
 let iconSerial=0;
 function sculptedShape(name){let defs='',index=0;const prefix='ohana-gem-'+(++iconSerial)+'-';const shape=(shapes[name]||shapes.flower).replace(/fill="(#[0-9a-fA-F]{6})"/g,(_,color)=>{const id=prefix+(index++);const n=parseInt(color.slice(1),16),r=n>>16,g=n>>8&255,b=n&255;const dark='#'+[r,g,b].map(v=>Math.round(v*.62).toString(16).padStart(2,'0')).join('');defs+='<linearGradient id="'+id+'" x1="0" y1="0" x2=".7" y2="1"><stop stop-color="#fff1ce"/><stop offset=".18" stop-color="'+color+'"/><stop offset=".65" stop-color="'+color+'"/><stop offset="1" stop-color="'+dark+'"/></linearGradient>';return 'fill="url(#'+id+')"';});return '<defs>'+defs+'</defs>'+shape;}
 function icon(name){const box=document.createElement('span');box.className='island-icon';box.setAttribute('aria-hidden','true');box.innerHTML='<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" fill="none" stroke="#275b64" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round">'+sculptedShape(name)+'</svg>';return box;}
