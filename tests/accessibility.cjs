...
(async()=>{
 ...
 for(const type of ['tictac','memory','checkers']){
  ...
  assert.equal(d.querySelectorAll('#game-head .table-player').length,2);assert.equal(d.querySelectorAll('#game-head .table-player .turn-check').length,1);assert(!d.querySelector('.gb-scores'));const cells=[...d.querySelectorAll('#board [data-i]')];assert(cells.every(c=>c.getAttribute('aria-label')));cells[0].focus();key(cells[0],'ArrowRight');assert.equal(d.activeElement.dataset.i,cells[1].dataset.i);await audit(type);
  if(type==='memory'){assert(cells.every(c=>c.getAttribute('aria-label').endsWith('face down')));w.run("previewGame.state.open=[0];renderGame()");assert.match(d.querySelector('[data-i="0"]').getAttribute('aria-label'),/Honu, face up/);assert(!d.querySelector('[data-i="1"]').getAttribute('aria-label').includes('Honu'));}
