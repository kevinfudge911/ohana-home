...
const html=fs.readFileSync('buddies-preview.html','utf8');let speaks=0,cancels=0,suspended=0,resumed=0;
const dom=new JSDOM(html,{url:'https://example.test',runScripts:'dangerously',pretendToBeVisual:true,beforeParse(w){w.SpeechSynthesisUtterance=function(text){this.text=text;};w.speechSynthesis={getVoices:()=>[{name:'Natural English',lang:'en-US'}],cancel:()=>cancels++,speak:u=>{speaks++;u.onstart?.();}};}});const w=dom.window,d=w.document;
try{
