...
assert.equal(Object.keys(c.roster).length,21);
assert.equal(new Set(Object.values(c.roster).map(x=>x.name)).size,21);
...
console.log('PASS: all 21 unique characters have four-character server IDs, portraits, animation strips and voice profiles.');
